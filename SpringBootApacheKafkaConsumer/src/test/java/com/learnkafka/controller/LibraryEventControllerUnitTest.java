 ...
 * com.learnkafka.producer.LibraryEventProducer; import
 ...
 * 
 * import static org.mockito.ArgumentMatchers.isA; import static
 ...
 * 
 * @Test void postLibraryEvent() throws Exception { //given Book book =
 * Book.builder() .bookId(123) .bookAuthor("Dilip")
 ...
 * ).thenReturn(null);
 * 
 * //expect mockMvc.perform(post("/v1/libraryevent") .content(json)
 * .contentType(MediaType.APPLICATION_JSON)) .andExpect(status().isCreated());
 * 
 * }
 * 
 * @Test void postLibraryEvent_4xx() throws Exception { //given
 * 
 * Book book = Book.builder() .bookId(null) .bookAuthor(null)
 * .bookName("Kafka using Spring Boot") .build();
 * 
 * LibraryEvent libraryEvent = LibraryEvent.builder() .libraryEventId(null)
 * .book(book) .build();
 ...
 * String json = objectMapper.writeValueAsString(libraryEvent);
 * when(libraryEventProducer.sendLibraryEvent_Approach2(isA(LibraryEvent.class))
 * ).thenReturn(null); //expect String expectedErrorMessage =
 * "book.bookAuthor - must not be blank, book.bookId - must not be null";
